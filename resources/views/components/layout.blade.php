                ...
                <ul class="navbar-nav me-auto mb-2 mb-md-0">
                    ...
                    <li class="nav-item dropdown">
                        ...
                        <ul class="dropdown-menu bg-dark" aria-labelledby="categoriesDropdown">
                            <li>
                                <a href="{{route('articles.index')}}" class="dropdown-item text-white">
                                    {{__('ui.allArticleDropdown')}}
                                </a>
                            </li>
                            @foreach($categories as $category)
                                <li>
                                    <a href="{{route('categoryShow', compact('category'))}}" class="dropdown-item text-white">
                                        {{$category->name}}
                                    </a>
                                </li>
                            @endforeach
                    ...
                    <li class="nav-item dropdown">
                        ...
                        <ul class="dropdown-menu bg-dark">
                            <li class="dropdown-item"><x-_en lang="en" /></li>
                            <li class="dropdown-item"><x-_ita lang="it" /></li>
                        </ul>
                    </li>
                ...
                <ul class="navbar-nav mb-2 mb-md-0 d-flex align-items-center">
                    ...
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle" role="button" data-bs-toggle="dropdown" aria-expanded="false" href="#">
                            {{Auth::user()->name}} <img src="{{Auth::user()->profile->picture==Storage::url('public/defaultUserPicture.webp') ? Auth::user()->profile->picture : Storage::url(Auth::user()->profile->picture)}}" alt="" style="height: 40px; width: 40px; border-radius: 20px">
                        </a>
                        <ul class="dropdown-menu">
                          ...
                          <li>
                            <form action="{{route('logout')}}" method="POST">
                                @csrf
                                <button class="dropdown-item">{{__('ui.logoutButton')}}</button>
                            </form>
                          </li>
                        </ul>
                    </li>
